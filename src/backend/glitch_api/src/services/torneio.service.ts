...
export class TorneioService {
  async addTorneio(dados: any): Promise<any> {
    ...
    try {
      let usuario = await models.Usuarios.findOne({
        where: { nickname: dados.usuario_responsavel },
        transaction,
      });
      if (!usuario) {
      ...
      let torneios = await models.Torneios.findAll({
        ...
        include: [
          ...
          },
          {
            ...
            as: "participantes",
            attributes: ["status", "usuario_id", "equipe_id"],
            ...
            include: [
              ...
              },
              {
                ...
                include: [
                  {
                    model: models.Usuarios,
                    as: "membros", // Nome definido no belongsToMany em index.models.ts
                    attributes: ["nickname"],
                    through: { attributes: [] }, // Remove dados da tabela intermediária do retorno
                  },
                ],
              },
            ],
      ...
      let torneio = await models.Torneios.findByPk(id, {
        ...
        include: [
          ...
          },
          {
            ...
            as: "participantes",
            where: { status: "APROVADO" },
            attributes: ["status", "dt_inscricao", "usuario_id", "equipe_id"],
            required: false,
            include: [
              {
                model: models.Usuarios,
              ...
              },
              {
                ...
                include: [
                  {
                    model: models.Usuarios,
                    as: "membros", // Alias definido em index.models.ts
                    attributes: ["nickname"],
                    through: { attributes: [] },
                  },
                ],
  ...
  async ingressarEmTorneioIndividual(
    ...
    try {
      ...
      if (!configInscricao) {
        throw new Error("Configuração de inscrição não foi encontrado");
      }
      ...
      if (
        countParticipantes >= configInscricao.dataValues.qtd_participantes_max
      ) {
  ...
  async ingressarEmTorneioEquipe(
    torneio_id: string,
    equipe_id: string,
  ): Promise<any> {
    let transaction = await sequelize.transaction();
    try {
      ...
      if (!configInscricao) {
        throw new Error("Config de Inscrição não foi encontrado");
      }
      ...
      if (!equipe) {
        throw new Error("Equipe não foi encontrado");
      }
    ...
    try {
      const relatorio = await models.EtapasPartida.findAll({
        ...
        order: [
          ...
          [
            { model: models.Partidas, as: "partidas" },
            { model: models.Chaveamentos, as: "chaveamentos" },
            "ordem",
            "ASC",
          ], // Ordem do jogo na chave
        ],
        include: [
          {
            ...
            include: [
              {
                model: models.Participantes,
                as: "participante_a",
                include: this.getParticipanteInclude(), // Helper para evitar repetição
              },
              {
                model: models.Participantes,
                as: "participante_b",
                include: this.getParticipanteInclude(),
              },
              {
                model: models.Participantes,
                as: "vencedor",
  ...
  /**
   * Helper interno para padronizar a busca de participantes (Solo ou Equipe)
   * com seus respectivos membros para validação no front.
   */
  private getParticipanteInclude() {
    return [
      {
        model: models.Usuarios,
        as: "usuario",
      ...
      },
      {
        ...
        include: [
          {
            model: models.Usuarios,
            as: "membros", // Alias conforme definido no index.models.ts
            attributes: ["nickname"],
            through: { attributes: [] },
          },
        ],
      ...
      partidas: etapa.partidas.map((partida: any) => ({
        ...
        confrontos: partida.chaveamentos.map((chave: any) => {
          ...
          return {
            ...
            vencedor: chave.vencedor_id
              ? chave.vencedor_id === chave.participante_a_id
                ? nomeA
                : nomeB
              : "Em aberto",
            status_pronto:
              chave.is_a_pronto && chave.is_b_pronto
                ? "Pronto para Jogar"
  ...
  async gerarPartidas(torneio_id: string) {
    ...
    try {
      ...
      await models.EtapasPartida.bulkCreate(gerado.etapasGeradas, {
        transaction,
      });
  ...
  private gerar(
    torneioId: string,
    participantes: ParticipantesAtributos[],
    numRodadas: number = 1,
  ) {
  ...
  async getPartidasDoJogador(usuarioId: string): Promise<any> {
    try {
      ...
      const chaveamentos = await models.Chaveamentos.findAll({
        ...
        order: [
          [{ model: models.Partidas, as: "partida" }, "dt_inicio", "DESC"],
        ],
      });
