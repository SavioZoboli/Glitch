...
class UsuarioService {
    ...
    try {
      ...
      let usuarios = await Models.Usuario.findAll({
        ...
        include: [
          ...
          {
            model: Models.TipoUsuario,    // Junta com a tabela TipoUsuario e tras os dados do campo attributes
            as: "tipoUsuario",
            attributes: ["id_tipo_usuario","descricao"],
          },
        ],
      });
  ...
  public async add(dados: DadosInclusaoUsuario): Promise<boolean | null> {
    ...
    try {
      ...
      let countTipo = await Models.TipoUsuario.count({
        where: { id_tipo_usuario: dados.tipo_usuario },
        transaction, // Informa a transacion que deve ser usada na consulta
      });

      // * Cria a pessoa
      let pessoa = await Models.Pessoa.create(
        {
          nome: dados.nome,
      ...
      if (pessoa && countTipo == 1) { // Se a pessoa foi adicionada e o tipo é válido
        // Cria o usuário
        let usuario = await Models.Usuario.create(
          {
            login: dados.login,
