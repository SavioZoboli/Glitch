...
export class CreateTournament {
  ...
  registrationBeforeTournamentValidator(): ValidatorFn {
    return (group: AbstractControl): ValidationErrors | null => {
      ...
      const r = new Date(registrationsDate.value);
      if (r > t) {
        registrationsDate.setErrors({ registrationAfterTournament: true });
        return { registrationAfterTournament: true };
      } else {
        if (registrationsDate.hasError('registrationAfterTournament')) {
          registrationsDate.setErrors(null);
        }
      }
  ...
  minLessOrEqualMaxValidator(): ValidatorFn {
    return (group: AbstractControl): ValidationErrors | null => {
      const min = group.get('minParticipants');
      const max = group.get('maxParticipants');
  ...
  get registrationAvailableControl() {
    return this.form.get('registrationAvailable') as FormControl;
  }
